...
```bash
PHASE_INFO=$(node "/home/rouby/space/.opencode/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "/home/rouby/space/.opencode/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "/home/rouby/space/.opencode/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
