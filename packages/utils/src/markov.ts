import starSystems from './names/starSystems';
...
export function generateName(type: keyof typeof nameSet) {
...
export function generateNames(type: keyof typeof nameSet, num: number) {
  const list = [];
...
function markovChain(type: keyof typeof nameSet) {
  const chain = chainCache[type];
  if (chain) {
    return chain;
  } else {
    const list = nameSet[type];
    if (list) {
