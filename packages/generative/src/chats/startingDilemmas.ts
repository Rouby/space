...

{
	...
	Leaders have a 50% shorter lifespan.\`
		...
		{
			role: "user" as const,
			content: `Now, please generate a new dilemma for the **The Origin** category. The effects should be broad, persistent, and "soft" – affecting things like population, influence, and research.`,
		},
	],
	format: z.object({
		dilemma: z.object({
	...
	assistantGeneration: (dilemma: {
		title: string;
		description: string;
		choices: { title: string; description: string }[];
	}) => ({
		role: "assistant" as const,
		content: `# ${dilemma.title}

${dilemma.description}

${dilemma.choices.map((c) => `- ${c.title}, ${c.description}`).join("\n\n")}
`,
	}),
	followUp1: (choiceId: string) => ({
		role: "user" as const,
		content: `I have choosen choice with ID "${choiceId}". Now please please generate the next dilemma for the follow-up **The Crucible** category. The effects should be "medium" impact, shaping your starting homeworld, early-game technologies, and political standing.`,
	}),
	followUp2: (choiceId: string) => ({
		role: "user" as const,
		content: `I have choosen choice with ID "${choiceId}". Now please please generate the next dilemma for the follow-up **The Launch** category. The effects should be "high" impact, determining your initial fleet composition, technological advantages, and diplomatic relationships.`,
	}),
};
